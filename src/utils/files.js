...
export const promptToSave = (list, logs) => {
  if (readline.keyInYN('Is the list fine? (Y)')) {
    ...
    updateLogs(logs);
    console.log(
      'Please paste the following message in slack channel #card-random-fika\n'
    );
    list.forEach((one, two) => console.log(`${one} - ${two}`));
  } else {
